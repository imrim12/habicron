...
describe('useHabicron (vue)', () => {
  ...
  it('omits control members unless controls: true', () => {
    const result = useHabicron(() => {}, { every: '1h' })
  ...
  it('exposes controls when requested', async () => {
    const cb = vi.fn()
    const job = useHabicron(cb, { controls: true, every: '10s' })
  ...
  it('does not start timers during SSR (no window)', async () => {
    ...
    try {
      const cb = vi.fn()
      const { counter } = useHabicron(cb, { every: '10s' })
